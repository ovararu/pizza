...
;(function($) {
	...
	$.fn.stepy = function(settings) {
		...
		var id			= $global.attr('id'),
			steps		= $global.children('fieldset:not(.buttons)'),
			size		= steps.size(),
			description	= '',
			title		= '',
			step;
        ...
        var finish = $global.find('input.finish'),
        	$this	= $global,
        	opt		= options;
        ...
        function getMaxStep(context, opt, clicked) {
          ...
          /*
              recode w/ ajax or html? jquery.validator.js 
...
//		}
	...
	function selectStep(context, index) {
		var id		= context.attr('id'),
			size	= context.children('fieldset:not(.buttons)').size(),
			step;
		...
		context
			.children('fieldset:not(.buttons)').hide()
		.end()
			.children('fieldset:not(.buttons)')
				.eq(index).show();
    ...
    function validate(context, index, options) {
    	...
    	var id		= context.attr('id'),
    		isValid	= true,
    		step	= context.children('fieldset:not(.buttons)').eq(index),
