...
(function($) {	
	...
	v = $.tools.validator = {
		...
		conf: {   
			...
			messageClass: 'inline-errors',		// error message element's class name
			offset: [0, 0], 
	...
	function Validator(inputs, form, conf) {		
		...
		$.extend(self, {
			...
			invalidate: function(errs, e) {
				...
				if(this.getForm().hasClass('formtastic')){ //hack
				  prefix = this.getForm().attr('class').split(" ")[1] //hack
				} //hack
				...
				if (!e) {
					...
					$.each(errs, function(key, val) {
					  ...
					  if (prefix != undefined) { //hack
  						var input = inputs.filter("[name='" + prefix + '[' + key + ']' + "']"); //hack
					  } else { //hack
