class Cartographer::Gmarker
  ...
  def to_js(marker_mgr_flag = false, marker_clusterer_flag = false)
    ...
    script << "#{@name} = new google.maps.Marker({title: #{@title.to_json}, position: new google.maps.LatLng(#{@position[0]}, #{@position[1]}), draggable: #{@draggable}, icon: #{@icon.name}}); \n"
    ...
    if @dblclick
      script << "google.maps.event.addListener(#{name}, 'dblclick', function(event) {#{@dblclick}});\n"
    end
